...
- **訓練運行狀態(2026-07-02,已暫停待續)**:
  ...
  - **checkpoint 存放(2026-07-05 整理後)**:成品=`zhou_yinuo_flux768_v2-000004.safetensors`(v2 ep4)。**.161 只留成品一顆**在 ComfyUI `loras\`(生圖用);.161 訓練輸出 checkpoint 全清。**完整備份在 G:**:`03_output\`(v1 六顆)、`05_curation_v2\output_v2\`(v2 六顆 + `zhou_yinuo_FINAL_v2ep4.safetensors`)。生圖用強度:典雅/定裝 1.0、寫實/濕髒 0.65–0.85。
  ...
  - **給檔案給使用者**:G: 是網路磁碟 `\\192.168.3.28\g`,markdown 絕對路徑連結點不開;改用純路徑,看圖給 .161 ComfyUI 的 `/view` HTTP 連結(把圖 upload 成 input 即可 serve 非產出圖)。見 [[feedback-file-links-plain-path]]。
- **周以諾寫真集(2026-07-03 進行中)**:v2ep4 LoRA 生 SFW 寫真集,存 `07_photobook\`(每場一子夾+`_meta\` 存每張 workflow JSON+prompt,見 [[feedback-save-workflow-per-image]])。已完成:01 純白棚拍(6)、02 居家慵懶(15)、03 豪宅絲質camisole(15,大廳/餐廳/廚房)。**服裝偏好:無袖細肩帶洋裝(碎花/純色/雪紡)+ 絲質 camisole 最適合周,使用者明確喜歡**。尺度=一般寫真、清爽端莊、不露點不情色(周設定 18 歲)。原規劃 20 場景。災難動態場景暫擱(工具難兼顧臉+動態+場景,PuLID 換臉可行但使用者覺得不夠;參考圖 img2img+PuLID 是最接近解法)。
